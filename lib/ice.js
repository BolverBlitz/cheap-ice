...
class IngressHistorySimulator extends IngressIceReplica {
     ...
     */
    async simulateHistory(outputDir, stepSeconds = 1) {
        if (!this.browser) await this.startBrowser();
        ...
        // Initialize Logic Engine (The Fix for Links/Fields)
        const simulator = new IngressStateSimulator(portals);

        // Calculate Map Center
        const avgLat = portals.reduce((sum, p) => sum + p.lat, 0) / portals.length;
        const avgLng = portals.reduce((sum, p) => sum + p.lng, 0) / portals.length;
        ...
        <head>
            ...
            <style>
                ...
                /* UI HIDING FIX: Hide standard Ingress Elements if they exist */
                ...
                window.renderState = (state, timeStr) => {
                    ...
                    state.links.forEach(l => {
                        ...
                        if(p1 && p2) {
                            ...
                            // Color link based on portal team
                            const team = p1.team === 'NEUTRAL' ? 'RES' : p1.team; // Fallback
        ...
        await this.page.evaluate(({ lat, lng }) => window.initMap(lat, lng), { lat: avgLat, lng: avgLng });

        // Wait for network to be idle to ensure tiles are loaded
        ...
        for (let time = startTime; time <= endTime; time += stepSeconds * 1000) {
            // Update Logic Engine
            ...
            if (time % 60000 === 0) console.log(`Rendered ${timeStr}`);
        }
